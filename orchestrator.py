...
from aggregator import MessageAggregator, BATCH_SIZE
...
class PipelineOrchestrator:
    ...
    def step_filter_and_aggregate(self):
        logger.info("Step 3: Filtering and aggregating genocidal messages...")
        try:
            aggregator = MessageAggregator()
            messages = aggregator.collect_genocidal_messages(limit=BATCH_SIZE)

            if messages:
                batch = aggregator.create_batch(messages)
                logger.info(
                    f"Created batch with {batch.count} messages (batch_id: {batch.batch_id})"
                )
                return aggregator, batch
            else:
                logger.info("No genocidal messages to aggregate")
                return aggregator, None
        except Exception as e:
            logger.error(f"Filter/aggregate failed: {e}")
            return None, None

    def step_publish(self, aggregator, batch) -> int:
        logger.info("Step 4: Publishing aggregated messages...")
        try:
            if not batch:
                logger.info("No batch to publish")
                return 0

            publisher = TelegramPublisher()
            success = publisher.publish_digest(batch.messages, batch.batch_id)

            if success:
                self.stats["published"] = batch.count
                logger.info(f"Published digest with {batch.count} messages")
                return batch.count
            else:
    ...
    async def run_full_pipeline(self):
        ...
        aggregator, batch = self.step_filter_and_aggregate()

        if aggregator and batch and aggregator.should_publish():
            self.step_publish(aggregator, batch)
    ...
    def run_publish_only(self):
        ...
        if batch:
            count = self.step_publish(aggregator, batch)
        else:
            count = 0
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["full", "sync", "classify", "publish"],
        default="full",
        help="Pipeline mode",
    )
